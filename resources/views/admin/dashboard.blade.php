    ...
    <div class="glass rounded-3xl p-6">
        ...
        <form class="mt-5 space-y-4" method="POST" action="{{ route('admin.tools.store') }}">
            ...
            <div>
                ...
                <input name="subtitle" required value="{{ old('subtitle') }}"
                       class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3"
            ...
            <div>
                ...
                <textarea name="highlights_text" rows="3"
                          class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3"
                          placeholder="Produce más contenido&#10;Monetiza con IA&#10;Todo listo para escalar">{{ old('highlights_text') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="extras_text" rows="5"
                          class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3"
                          placeholder="Cursos completos&#10;Masterclass VIP&#10;Actualizaciones constantes">{{ old('extras_text') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="audience" rows="2"
                          class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3"
                          placeholder="Ideal para creadores, marketers y emprendedores.">{{ old('audience') }}</textarea>
            </div>

            <div class="grid gap-3 sm:grid-cols-3">
                <div>
                    <label class="text-sm text-white/80">Mensual ($)</label>
                    <input name="price_monthly" type="number" step="0.01" min="0" value="{{ old('price_monthly') }}"
                ...
                <div>
                    ...
                    <input name="price_semestral" type="number" step="0.01" min="0" value="{{ old('price_semestral') }}"
                ...
                <div>
                    ...
                    <input name="price_anual" type="number" step="0.01" min="0" value="{{ old('price_anual') }}"
            ...
            <div class="grid gap-3 sm:grid-cols-3">
                <div>
                    ...
                    <input name="savings_semestral" type="number" step="0.01" min="0" value="{{ old('savings_semestral') }}"
                ...
                <div>
                    ...
                    <input name="savings_anual" type="number" step="0.01" min="0" value="{{ old('savings_anual') }}"
            ...
            @forelse($tools as $tool)
                ...
                <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <div class="text-xs text-white/70">
                                {{ $tool->tag ?? 'PACK' }} · Orden: {{ $tool->sort_order }}
                                @if($tool->badge_text) · <span class="text-cyan-200">{{ $tool->badge_text }}</span> @endif
                            </div>
                            <div class="font-bold">{{ $tool->title }}</div>
                            <div class="text-white/70 text-sm">{{ $tool->subtitle }}</div>
                            <div class="text-xs text-white/50 mt-1">
                                Mensual: ${{ $tool->price_monthly ?? '-' }} · Sem: ${{ $tool->price_semestral ?? '-' }} · Anual: ${{ $tool->price_anual ?? '-' }}
                            </div>
                            <div class="text-xs text-white/50 mt-1">
                    ...
                    <details class="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                        ...
                        <form class="mt-4 space-y-4" method="POST" action="{{ route('admin.tools.update', $tool) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="grid gap-3 sm:grid-cols-2">
                                <div>
                                    <label class="text-sm text-white/80">Tag</label>
                                    <input name="tag" value="{{ old('tag', $tool->tag) }}"
                                           class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">
                                </div>

                                <div>
                                    <label class="text-sm text-white/80">Badge</label>
                                    <input name="badge_text" value="{{ old('badge_text', $tool->badge_text) }}"
                                           class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">
                                </div>
                            </div>
                            ...
                            <div>
                                <label class="text-sm text-white/80">Título</label>
                                <input name="title" required value="{{ old('title', $tool->title) }}"
                                       class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">
                            </div>

                            <div>
                                <label class="text-sm text-white/80">Subtítulo</label>
                                <input name="subtitle" required value="{{ old('subtitle', $tool->subtitle) }}"
                                       class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">
                            </div>

                            <div>
                                <label class="text-sm text-white/80">Descripción corta</label>
                                <input name="short_desc" value="{{ old('short_desc', $tool->short_desc) }}"
                                       class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">
                            </div>
                            ...
                            <div>
                                <label class="text-sm text-white/80">Highlights (1 por línea)</label>
                                <textarea name="highlights_text" rows="3"
                                          class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">{{ old('highlights_text', $highlightsText) }}</textarea>
                            </div>

                            <div>
                                <label class="text-sm text-white/80">Incluye (1 por línea: Label|Texto)</label>
                                <textarea name="includes_text" rows="6"
                                          class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">{{ old('includes_text', $includesText) }}</textarea>
                            </div>

                            <div>
                                <label class="text-sm text-white/80">Extras (1 por línea)</label>
                                <textarea name="extras_text" rows="5"
                                          class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">{{ old('extras_text', $extrasText) }}</textarea>
                            </div>

                            <div>
                                <label class="text-sm text-white/80">¿Para quién es?</label>
                                <textarea name="audience" rows="2"
                                          class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">{{ old('audience', $tool->audience) }}</textarea>
                            </div>
                            ...
                            <div class="grid gap-3 sm:grid-cols-3">
                                <div>
                                    ...
                                    <input name="price_monthly" type="number" step="0.01" min="0"
                                           value="{{ old('price_monthly', $tool->price_monthly) }}"
                                           class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">
                                </div>
                                <div>
                                    <label class="text-sm text-white/80">Semestral</label>
                                    <input name="price_semestral" type="number" step="0.01" min="0"
                                           value="{{ old('price_semestral', $tool->price_semestral) }}"
                                           class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">
                                </div>
                                <div>
                                    <label class="text-sm text-white/80">Anual</label>
                                    <input name="price_anual" type="number" step="0.01" min="0"
                                           value="{{ old('price_anual', $tool->price_anual) }}"
                                           class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">
                                </div>
                            </div>
                            ...
                            <div class="grid gap-3 sm:grid-cols-3">
                                <div>
                                    ...
                                    <input name="savings_semestral" type="number" step="0.01" min="0"
                                           value="{{ old('savings_semestral', $tool->savings_semestral) }}"
                                           class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">
                                </div>
                                <div>
                                    <label class="text-sm text-white/80">Ahorro anual</label>
                                    <input name="savings_anual" type="number" step="0.01" min="0"
                                           value="{{ old('savings_anual', $tool->savings_anual) }}"
                                           class="mt-1 w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3">
                                </div>
                                <div>
                                    <label class="text-sm text-white/80">Orden</label>
                            ...
                            <label class="flex items-center gap-2 text-white/80">
                                <input type="checkbox" name="is_active" {{ $tool->is_active ? 'checked' : '' }}>
                                Activa
                            </label>
